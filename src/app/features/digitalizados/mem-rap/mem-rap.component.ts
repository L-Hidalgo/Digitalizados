...
@Component({
  selector: "app-mem-rap",
  templateUrl: "./mem-rap.component.html",
  styleUrls: ["./mem-rap.component.css"],
})
export class MemRapComponent implements OnInit {
  ...
  openModalCrearCarpeta() {
    const dialogRef = this.dialog.open(CrearCarpetaComponent, {
      width: "350px",
      data: { parentId: this.parentId, tipoDocumentoFile: 2 },
    });
  ...
  getListDataMemoRap() {
    ...
    this.fileService.listarMemoRap(query, paginationOptions).subscribe(
      (resp) => {
        if (resp.objetosList) {
    ...
    this.fileService.downloadCarpeta(fileId).subscribe(
      ...
      (error) => {
        ...
        if (error.status === 404) {
          text =
            error.error?.error ||
            "No se encontró la carpeta o la carpeta está vacía.";
        } else if (error.status === 500) {
          text =
            "Error interno del servidor, no se pudo crear el archivo zip. Por favor, contacta al soporte.";
        } else {
