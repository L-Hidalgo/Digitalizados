...
export class FormacionesService {
  ...
  buscarPorPersonaId(personaId: number) {
    return this.http.get<RespuestaObjeto<Formacion>>(`http://127.0.0.1:8000/${this.path}/${personaId}/by-persona-id`);
  }

  createUpdateFormacion(data: Partial<Formacion>) {
    return this.http.put<RespuestaObjeto<Formacion>>(`http://127.0.0.1:8000/${this.path}`, data);
  }
}
